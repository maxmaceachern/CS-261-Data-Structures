...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	int index;
	if (source == destination)                  //If source is the same as destination, a path exists
        return 1;
    cirListDeque *searchDeq = malloc(sizeof(cirListDeque));    //Allocate memory for the search deque
    initCirListDeque(searchDeq);                               //Initialize the deque
    addBackCirListDeque(searchDeq, source);                     //Add initial vertex to the deque

    while (!isEmptyCirListDeque(searchDeq)){                    //Empty stack implies there are vertices left to search
        Vertex *current = backCirListDeque(searchDeq);          //DFS works off the top of the stack
        removeBackCirListDeque(searchDeq);                      //Pop the top of the stack to work with
        current->isVisited = 1;                                 //Mark that it has been visited
        if (current == destination)                             //Check to see if the current vertex is where we want to go
            return 1;
        for (index = 0;index < current->numNeighbors; index++){  //Work through neighbors
            if (!(current->neighbors[index]->isVisited))
                addBackCirListDeque(searchDeq, current->neighbors[index]);   //Add neighbors to deque
        }
    }
    removeAllCirListDeque(searchDeq);       //Only get here if there is no path
    free(searchDeq);
    clearVisited(g);
	return 0;
}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    while (!isEmptyCirListDeque(searchDeq)){                    //Empty stack implies there are vertices left to search
        Vertex *current = frontCirListDeque(searchDeq);         //DFS works off the front of the queue
        removeFrontCirListDeque(searchDeq);                      //Pop the front of the queue to work with
        current->isVisited = 1;                                 //Mark that it has been visited
        if (current == destination)                             //Check to see if the current vertex is where we want to go
            return 1;
        for (index = 0;index < current->numNeighbors; index++){  //Work through neighbors
