...
int main (int argc, const char * argv[]) {
	...
	if (fileptr == NULL)
	{
		printf("The file %s is not there, please try again!", filename);
		return 1;
	}
	timer = clock();
	...
	while (1)
	{
		...
		if (word == NULL)
			break;
		int *wordNum = atMap(hashTable, word);
		if (wordNum == NULL)
			insertMap(hashTable, word, 1);
		else{
			insertMap(hashTable, word, *wordNum + 1);
		}
		free(word);
	}
	...
	for(index = 0; index < capacity(hashTable); index++){
		tempHash = hashTable->table[index];
		while(tempHash != 0){
			printf("%s: ", tempHash->key);
			printf("%d\n", tempHash->value);
